...
public class CreationScreen extends Screen {
		...
		if (this.step == 0) {// Race selection
			...
			for (int i = 0; i < 6; i++) {
				...
				if (conf.clicked) {
					...
					this.chac.getSkillManager().fillWithStats(this.chac.getStats());
					next(1);
				}
			}
		} else if (this.step == 6) {
			System.out.println("Moving on the background selection");
			setScreen(new BackgroundScreen(chac));
		}
	}

	@Override
	/**
	 * Basic render function called from the framework
	 */
	public void render(Graphics g) {
		...
		if(this.step < 6){
			...
			if(this.step == 0){//Select race
				...
				Font.instace.draw(r, words, (DnD.w-(words.length()*7))/2, yoWord);
			}else if(this.step == 3){// Point picking stats
			...
			String words = "Please wait";
			Font.instace.draw(r, words, (DnD.w-(words.length()*7))/2, (DnD.h-8)/2);
		}
	}
	...
	public void setupStep() {
		...
		if (this.step == 0) {// Chose a race
			...
			String[] abilitys = new String[] { "STRength", "DEXterity", "WISdom", "CONstitution", "INTelligence",
					"CHRarisma" };
			...
			for (int i = 0; i < 6; i++) {
				...
				for (int j = 0; j < 2; j++) {
					this.ui.add(new Button("BTN" + i + ":" + j, j == 1 ? "+" : "-", xo + (j == 0 ? -24 : 72),
							yo + step * i));
				}
			}
			...
			saveTempFile();
			next(2);
		} else if (this.step == 5) {// Race & Class temp load point
			this.chac.getSkillManager().fillWithStats(this.chac.getStats());
			next(1);
		}
		this.inputdelay = 30;
